...
SYSTEM_PROMPT = (
    ...
    "- Never write a status code in summary, restated_reason or next_questions.\n"
    "  Those fields are English; the status field carries the code. Say\n"
    "  \"computed\" for COMPUTABLE_READY, \"withheld until the missing laws are\n"
    "  declared\" for WITHHELD_MISSING_DECLARED_LAW, and \"withheld because no\n"
    "  closing relation exists\" for WITHHELD_NO_DECLARED_LAW.\n"
    "- An output whose status is WITHHELD_NO_DECLARED_LAW has no closing relation\n"
    "  at all. No declaration releases it. Never ask which law it needs and never\n"
    "  suggest one; the honest question is whether the caller can define such a\n"
    "  relation, or should drop the output from scope.\n"
